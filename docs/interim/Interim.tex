...
\begin{document}
    ...
    in Haskell to evaluate our recipes or even create a list of steps \cite{hutton-fold}.
    
    % \begin{align}
    %     \llbracket \texttt{Ingredient s} \rrbracket = \texttt{GET (Ingredient s)} \\
    %     \llbracket \texttt{Heat t r} \rrbracket = \texttt{Node (PlaceInHeat r) [
    %         Node (Preheat t) [\llbracket \texttt{r} \rrbracket]]} \\
    %     \llbracket \texttt{Wait t} \rrbracket = \texttt{Leaf DoNothing} \\
        
    % \end{align}

    While a recipe action is described as a list of recipes above and a recipe process is
    a mapping from time to such a list, tt may turn out that some sort of tree structure is
    more appropriate. One such representation could be the following:
    ...
    That is at a given node N, the set of actions one could do next are the children of
    that node. Our function \texttt{RP :: Time -> RA} would therefore give the list of
    nodes at the depth equal to the time provided. \\
